...
import { TableInvitationService } from '../table-invitation/table-invitation.service';
...
export class CharacterService extends ApiRessourceService<Character> {
  ...
  acceptCharacterToTable$(characterId: number): Observable<CharacterFullDTO> {
    ...
    const headers = this.getHeaders();
    return this._http
      .patch<CharacterFullDTO>(
        this._BASE_URL_NEW + `/patch/${characterId}`,
        acceptedValue,
        { headers }
      )
      .pipe(
        tap((modifiedCharacter) => {
          this._characterOnHoldList = this._characterOnHoldList.filter(
            (character: CharacterAvatarDTO) =>
              character.id !== modifiedCharacter.id
          );
          this._tableCharacterOnHoldList$.next(this._characterOnHoldList);
        })
      );
