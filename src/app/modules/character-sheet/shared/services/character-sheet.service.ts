...
export class CharacterSheetService {
  ...
  constructor(
    ...
    this.listener.receiveFieldFrom(this.infoAutoGenerated$);
  }
        ...
        map((race: Race) => {
          if (race && gender && tmpSheet.heightModifierRolled) {
            if (!tmpSheet.weightModifierRolled) {
              tmpSheet.weightModifierRolled = DiceService.roll(race.modWeight);
              this.infoAutoGenerated$.next({
                index: 'weightModifierRolled',
                value: tmpSheet.weightModifierRolled.toString()
              });
            }
            return (race.baseWeight[gender] + (Number(tmpSheet.heightModifierRolled)) * tmpSheet.weightModifierRolled / 5).toFixed(0);
          }
          else {
            return "";
          }
